"""Simple Quiz Maker V.074
   ...
   File dependencies:
   ------------------
   quiz-runner-v0-74.exe
   quiz-help.txt
   help.txt
   sqm-example-logo.png
   ttips.py
...
import subprocess
from tkinter import Button, DISABLED, E, END, Entry, filedialog
from tkinter import Label, LabelFrame, Menu, messagebox, NORMAL
...
class Bg:
    ...
    orig_dir = os.getcwd()
    window_title = 'Simple Quiz Maker V0.74'
...
def file_checks():
    ...
    if not os.path.isfile('quiz-runner-v0-74.exe'):
        mf_msg = 'The file quiz-runner-v0-74.exe is missing.'
...
music_msg_lbl = Label(music_msg_frame,
                      ...
                      text='  To add images and music to your quiz, see help. ')
music_msg_lbl.grid(row=7, column=0, sticky=W+E)


def load_example_logo():
    "Load and display example logo."""
...
def create_dir():
    ...
    if not check_dir:
        ...
        dest = Bg.users_dir+str(r'\quiz-runner-v0-74.exe')
        copyfile('quiz-runner-v0-74.exe', dest)
        ...
        os.rename('quiz-runner-v0-74.exe', Bg.users_quiz_name+'.exe')
    ...
    check_dir = os.path.isdir('images')
    if not check_dir:
        os.makedirs('images')

    check_dir = os.path.isdir('music-q')
    if not check_dir:
        os.makedirs('music-q')
...
def check_qa():
    ...
    with open(r'c:\qa-check.txt', 'w') as f:
    ...
    webbrowser.open(r'c:\qa-check.txt')
...
def about_menu():
    ...
    messagebox.showinfo('Program Information', 'Simple Quiz Maker V0.74\n'
                        'Freeware, but (c) Steve Shambles, Oct 2020')
